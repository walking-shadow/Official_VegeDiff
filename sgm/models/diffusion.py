...
class DiffusionEngine(pl.LightningModule):
    ...
    def init_from_dict( # init SD-xL from safetensors
        self, model, model_state_dict, ignore_keys=None
    ) -> None: 
        model_new_ckpt=dict()
        for i in model_state_dict.keys():
            if "model.diffusion_model." in i:
                model_new_ckpt[i.replace("model.diffusion_model.", "")] = model_state_dict.get_tensor(i)
        sd=model_new_ckpt
        keys = list(sd.keys())
        for k in keys:
            if ignore_keys:
