...
from .video_module.motion_module import get_motion_module, get_motion_module_layer
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        # self attention: x: b*t,h*w,c, context(not using): b,t,d, others:default
        # cross attention: x: b*h*w,t,c, context: b*h*w,t,c, others:default
        h = self.heads
        ...
        k = self.to_k(context)
        v = self.to_v(context)
        # print(f'context before to k:{context[0][0]}')
        # print(f'corss atten q:{q[0][0]}')
        # print(f'corss atten k:{k[0][0]}')
        # print(f'corss atten v:{v[0][0]}')
        ...
        if attn_type == 'flash':
            q, k, v = map(lambda t: rearrange(t, "b n (h d) -> b n h d", h=h), (q, k, v)) # for flash
        ...
        if attn_type == 'flash':
            ...
            causal = False if self.context_dim is None else True  # 如果使用的是self-attention,那么是自注意力，不需要额外的attention mask，否则需要
            # # print(f'q:{q.shape}, k:{k.shape}, v:{v.shape}')
            # print(f'corss atten q before flash attn:{q[0][0]}')
            # print(f'corss atten k before flash attn:{k[0][0]}')
            # print(f'corss atten v before flash attn:{v[0][0]}')

            out=flash_attn_func(q, k, v, dropout_p=0.0, softmax_scale=None, causal=causal)  # 设置为True，使得图像像素点只能看到当前和之前的condition image
            # print(f'cross attn out:{out[0][0]}')
            # print("out", out.shape)
        elif attn_type == "pytorch":
        ...
        del q, k, v
        if attn_type == 'flash':
            out = rearrange(out, "b n h d -> b n (h d)", h=h) # for flash
        elif attn_type == "pytorch":
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn2 = attn_cls(
            ...
            dropout=dropout,
            backend=sdp_backend,
        )  # is self-attn if context_dim is none
    ...
    def forward(
        ...
        # x: b,t,h,w,c, context: b,t,d, others:default
        kwargs = {"x": x}
    ...
    def _forward(
        ...
        # x: b,t,h,w,c, context: b*h*w,t,d, others:default
        b,t,h,w,c = x.shape
        x = rearrange(x, "b t h w c-> (b t) (h w) c").contiguous() # BT HW C
        x = (
            self.attn1(
                self.norm1(x),
        ...
        # TODO ZSJ 想办法在这里把x从b*t,h*w,c变成b*h*w,t,c
        x = rearrange(x, "(b t) (h w) c -> (b h w) t c", b=b, t=t, h=h, w=w).contiguous()
        x = (
            self.attn2(
        ...
        x = rearrange(x, "(b h w) t c -> (b t) (h w) c", b=b, t=t, h=h, w=w).contiguous()
...
class SpatialTransformer(nn.Module):
    ...
    """
    # TODO ZSJ 把这里改成先使用SEER的那种时空注意力结构，即时间上序列可见，空间上全局可见；
    # 然后再用crossattention,让某时刻的图像只能看到当前时刻和之前的condition
    ...
    def forward(self, x, context=None):
        ...
        # x = b,t,c,h,w, context = b*h*w,t,d, list
        # if not isinstance(context, list):
        #     context = [context]
        b, t, c, h, w = x.shape
        # print(f'spatial transformer x origin:{x[0][0][0][0]}')
        # 找到相同大小的context，然后放入到网络中计算
        for context_d in context:
            if context_d.shape[0] == b*h*w:
                context_using = context_d
                break
        # print(f'spatial transformer context_using origin:{context_using[0][0]}')
        
        # print(f'spatial transformer x temporal:{x[0][0][0][0]}')


        # 交叉时序注意力
        # TODO ZSJ 如果预测变量在Unet里面下采样了，那么condition image也要同步下采样，
        # 已经在放入网络之间就让condition image和预测变量同样大小了
        # bhw_c, t_c, c_c = context.shape
        # if bhw_c == 4*b*h*w:
        #     context = rearrange(context, '(b h w) t c -> (b t) c h w', b=b, t=t, h=h*2, w=w*2).contiguous()
        #     context = torch.nn.functional.interpolate(context, scale_factor=0.5)
        #     context = rearrange(context, '(b t) c h w -> (b h w) t c',  b=b, t=t).contiguous()
        ...
        if not self.use_linear:
            x = self.proj_in(x)
        x = rearrange(x, "b c h w -> b (h w) c").contiguous() # BT HW C
        if self.use_linear:
            x = self.proj_in(x)
        x = rearrange(x, "(b t) (h w) c -> b t h w c", b=b, t=t, h=h, w=w).contiguous() # B T H W C
        for i, s_block in enumerate(self.transformer_blocks):
            # if i > 0 and len(context) == 1:
            #     i = 0  # use same context for each block
            # x = s_block(x, context=context[i])
            x = s_block(x, context=context_using)
            # x = rearrange(x.view(b, t, h, w, c), "b t h w c -> b c t h w")
            # x = t_block(x, temb=None, encoder_hidden_states=None)
            # x = rearrange(x, "b c t h w -> (b t) (h w) c")
        
        # print(f'spatial transformer x corss:{x[0][0]}')
        if self.use_linear:
            x = self.proj_out(x)
        ...
        output = x + x_in
        # print(f'spatial transformer x out:{x[0][0][0][0]}')

        # 先用单点时序注意力，让某时刻的图像像素点能够知道自己过去是什么状态，然后在环境因素的影响下，预测未来的状态
        # 单点时序注意力
        if self.temporal_block_layers != 0: # open temporal_block_layer
